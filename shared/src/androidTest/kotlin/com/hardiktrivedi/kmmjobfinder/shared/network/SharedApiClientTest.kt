...
class SharedApiClientTest {
    ...
    @Mock
    private lateinit var coreHttpClient: HttpClient
    ...
    @Test
    fun allAffectedCountriesReturnsListOfCountries() {

        GlobalScope.launch {
            Mockito.`when`(coreHttpClient.get(anyString()) as List<JobInfo>)
                .thenReturn(listOf(job))

            val jobs = SharedApiClient().getJobInfo()
            assertEquals(listOf(job), jobs)
            verify(coreHttpClient).get("https://jobs.github.com/positions.json?description=android&full_time=true&location=uk")
        }
    }
